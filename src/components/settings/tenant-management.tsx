            ...
            <div key={tenant.id} className="border rounded-lg p-4">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    {statsByTenant[tenant.id] && (
                      <div className="mt-2 text-sm text-gray-600 flex flex-wrap gap-4">
                        <div>{lt('Docs')}: <span className="font-medium">{statsByTenant[tenant.id].documents}</span></div>
                        <div>{lt('Tx')}: <span className="font-medium">{statsByTenant[tenant.id].transactions}</span></div>
                        <div>{lt('Banks')}: <span className="font-medium">{statsByTenant[tenant.id].bank_accounts}</span></div>
                        <div>{lt('Users')}: <span className="font-medium">{statsByTenant[tenant.id].users}</span></div>
                      </div>
                    )}
                </div>
                <div className="flex gap-2 flex-wrap">
                  ...
                  <Button variant="outline" size="sm" onClick={() => { setSelectedTenant(tenant); setEditOpen(true) }}>
                    {lt('View')}
                  </Button>
